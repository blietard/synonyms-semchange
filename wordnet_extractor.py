...
from params import WORDNET_DB_FOLDER, WORDS_FOLDER

parser = argparse.ArgumentParser(description='Read WordNet database.')
...
with open(WORDNET_DB_FOLDER + 'index.' + pos, 'r', encoding='utf-8') as f:
    nb_line = 0
    while True:
        line = f.readline()
        if line == '':
...
with open(f'{WORDS_FOLDER}/wordnet/synsets_by_ID_{args.pos}.pkl','wb') as pkl_f:
    pickle.dump(obj=synsets, file=pkl_f)
with open(f'{WORDS_FOLDER}/wordnet/synsets_by_words_{args.pos}.pkl','wb') as pkl_f:
    pickle.dump(obj=words_and_synonyms, file=pkl_f)
df.to_csv(f'{WORDS_FOLDER}/wordnet/words_{args.pos}.csv',sep='\t',index=True)